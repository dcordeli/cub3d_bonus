...
t_win		*win_init(t_pars *p, int argc, int i)
{
	t_win	*win;
	int		wid;
	int		heig;
...
t_plr		*plr_init(t_all *all, int i, int j, char c)
{
	...
	plr->svitok = 0;
	plr->ziri = 0;
	all->flo.path = "./textures/Moss.xpm";
	all->flo.img = mlx_xpm_file_to_image(all->win->mlx,
		all->flo.path, &all->flo.w, &all->flo.h);
	all->flo.addr = mlx_get_data_addr(all->flo.img, &all->flo.bpp,
		&all->flo.line_l, &all->flo.en);
	return (plr);
}
		...
		while (all->map[i][j] != '\0')
		{
			if (ft_isplayer(all->map[i][j]) == 1)
				if (!(all->plr = plr_init(all, i, j, all->map[i][j])))
					return (-1);
			if (ft_strchr("23456789", all->map[i][j]) != NULL)
				all->win->s_num[all->map[i][j] - '2']++;
			j++;
		}
		j = 0;
		i++;
...
t_all		*all_init(t_pars *p, int argc)
{
	...
	if (!(all = (t_all *)malloc(sizeof(t_all))))
		return (NULL);
	if (!(all->win = win_init(p, argc, -1)))
		return (NULL);
	all->map = p->map;
	if (player_init(all) == -1 || spr_init(all, -1, -1) == -1)
		return (NULL);
